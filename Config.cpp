...
Settings::Settings()
  : magic_bytes("magic_bytes", false, 0, 0xdeadbeef, 0, 0, "Magic byte to identify eeprom was initialized"),
    ...
    canbus_address("canbus_address", true, 0, 1, 0, 0, "Can bus address"),
    ...
    charger_cycle_v_setpoint("charger_cycle_v_setpoint", true, 0.0f, 4.17f, 2.5f, 4.25f, "cycle charger to force a charging cycle"),
    ...
    cooling_lowt_setpoint("cooling_lowt_setpoint", true, 0.0f, 25.0f, -20.0f, 65.0f, "threashold at wich coolant pump gradually increases duty up to max"),
    cooling_hight_setpoint("cooling_hight_setpoint", true, 0.0f, 35.0f, -20.0f, 65.0f, "threshold at wich coolant pump is at maximum duty"),
    over_t_setpoint("over_t_setpoint", true, 0.0f, 35.0f, 30.0f, 60.0f, "Triggers Over T error; Tesla seam to allow reaching 45C while supercharging; in discharge, 60C is ok"),
    under_t_setpoint("under_t_setpoint", true, 0.0f, -10.0f, -40.0f, 10.0f, "Triggers Under T error"),
    warn_t_offset("warn_t_offset", true, 0.0f, 5.0f, 0.5f, 10.0f, "TODO, issue a warning on OLED and serial console if T is that close to a OT or UT fault"),
...
template<>
void ParamImpl<uint32_t>::setRawBytes(uint8_t* bytes) {
  value = *(uint32_t*)bytes;
}

template<>
void ParamImpl<int32_t>::setRawBytes(uint8_t* bytes) {
  value = *(int32_t*)bytes;
}

template<>
void ParamImpl<float>::setRawBytes(uint8_t* bytes) {
  value = *(float*)bytes;
}
